...
bool MoveItKinematics::initialize(const std::shared_ptr<rclcpp::Node> & node, const std::string & group_name)
{
  node_ = node;
  ...
  // There should not be multi-threading here)
  ...
  auto robot_description_semantic =
    ...
    {
      RCLCPP_ERROR(node_->get_logger(), "Could not retrieve the robot description parameter.");
      return false;
    }
  return true;
}
